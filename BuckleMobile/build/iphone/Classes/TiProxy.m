...
//Rare exceptions to indicate a bug in the _bucklemobile code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_bucklemobile = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_bucklemobile]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.bucklemobile.module"
