...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kBotsford_Bryan_Project3UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kBotsford_Bryan_Project3UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Botsford_Bryan_Project3 app using the node.js CLI
